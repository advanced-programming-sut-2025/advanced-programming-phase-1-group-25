    ...
    public String craftingCraft(String itemName, Game game) {
        ...
        if (game.getCurrentPlayer().getInventory().getCapacity() <= game.getCurrentPlayer().getInventory().getItems().size()) {
            ...
            if (player.getInventory().hasItem(App.getItemDefinition("fish_smoker_recipe").getId())) {
                ...
                return "You don't have teh recipe to create fish smoker.";
            }
...
//        StringBuilder result = new StringBuilder();
//        if (definition.getType().toString().equalsIgnoreCase("crafting") || definition.getType().toString().equals("shopRecipe")) {
//            Map<ItemAttributes, Object> baseAttributes = definition.getBaseAttributes();
