...
import org.example.Views.InGameMenus.CookingMenu;
import org.example.Views.InGameMenus.CraftingMenu;
...
public class CookingMenuController {
    ...
    public String putRef(String id, Game game) {
        Player player = game.getCurrentPlayer();
        ItemDefinition itemDefinition = App.getItemDefinition(id);
        if (!player.getInventory().hasItem(itemDefinition.getId())) {
            return "You don't have " + itemDefinition.getDisplayName() + ".";
        } else if ((itemDefinition.getType().toString().equalsIgnoreCase("food") || itemDefinition.getType().toString().equalsIgnoreCase("shop_recipe"))) {
    ...
    public String pickRef(String id, Game game) {
        Player player = game.getCurrentPlayer();
        ItemDefinition itemDefinition = App.getItemDefinition(id);
